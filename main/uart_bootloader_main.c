...
void init() {
  ...
  ESP_ERROR_CHECK(
      uart_set_pin(ECHO_UART_PORT_NUM, ECHO_TEST_TXD, ECHO_TEST_RXD, -1, -1));
...
bool wait_for_ack(void) {
  ...
  uart_read_bytes(ECHO_UART_PORT_NUM, &ack, 1, pdMS_TO_TICKS(200));
  if (ack == 0x79) {
    // ESP_LOGI(TAG, "  ACK");
    return true;
  } else
...
void sync_bootloader_v2() {
  ...
  while (1)
  {
    ...
    int len = uart_read_bytes(ECHO_UART_PORT_NUM, &response, 1,
  ...
  // Give STM32 time after reset/BOOT0 changes
  vTaskDelay(pdMS_TO_TICKS(300));

  //   sync_bootloader_v3();

  for (int attempt = 0; attempt < 50; attempt++) {
    ESP_LOGI(TAG, "Sync attempt %d", attempt + 1);

    // Send sync byte
    uart_write_bytes(ECHO_UART_PORT_NUM, (const char *)&sync_byte, 1);

    uart_wait_tx_done(ECHO_UART_PORT_NUM, pdMS_TO_TICKS(20));

    // Wait for ACK
    int len =
        uart_read_bytes(ECHO_UART_PORT_NUM, &response, 1, pdMS_TO_TICKS(200));

    if (len == 1) {
      ESP_LOGI(TAG, "RX = 0x%02X", response);

      if (response == 0x79) {
        ESP_LOGI(TAG, "STM32 bootloader ACK received");
        break;
      }

      if (response == 0x1F) {
        ESP_LOGW(TAG, "STM32 bootloader NACK received");
      }
    } else {
...
void flush() {
  ...
  while (uart_read_bytes(ECHO_UART_PORT_NUM, flush_buf, sizeof(flush_buf),
...
int read_uart(void *buf, uint32_t length) {
  return uart_read_bytes(ECHO_UART_PORT_NUM, buf, length,
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
  ...
  case MQTT_EVENT_DATA:
    ...
    if (event->topic_len == strlen("testtopic/221m") &&
      ...
      printf("LEN=%d, crc=%d\n", data_len, expected_crc_id);
      if (expected_crc_id == -1) {
        ESP_LOGE(TAG, "Unable to find crc, skipping event");
        break;
      }

      uint8_t *bare_led_bin = malloc(data_len);
      ...
      memcpy(bare_led_bin + event->current_data_offset, event->data,
             event->data_len);

      if (event->current_data_offset + event->data_len ==
          event->total_data_len) {
        uint32_t dump_crc = esp_rom_crc32_le(0, bare_led_bin, data_len);
        ESP_LOGI(TAG, "dump crc %d", dump_crc);
        ESP_LOGI(TAG, "expected crc %d", expected_crc_id);
        const uint32_t bare_led_bin_len = event->total_data_len;

        if (expected_crc_id == dump_crc) {
          // ready to flash
          // read version
          ...
          uint8_t addr_bytes[4] = {(addr >> 24) & 0xFF, (addr >> 16) & 0xFF,
          ...
          // const int r = uart_read_bytes(ECHO_UART_PORT_NUM,
          // &full_dump[addr_offset], chunk_len, pdMS_TO_TICKS(1000));
          ...
          if (addr == 0x08000200) {
            ...
            version = (uint32_t)full_dump[addr_offset + 0] |
          ...
          uint32_t version_fw = (uint32_t)bare_led_bin[addr_offset + 0] |
          ...
          for (addr_offset = 0; addr_offset < bare_led_bin_len;
          ...
          uint32_t actual_crc =
              esp_rom_crc32_le(0, full_dump, bare_led_bin_len);
          uint32_t flash_fw_crc =
              esp_rom_crc32_le(0, bare_led_bin, bare_led_bin_len);

          ESP_LOGI(TAG, "BEFORE FLASH actual_crc %d", actual_crc);
          ESP_LOGI(TAG, "BEFORE FLASH flash_fw_crc %d", flash_fw_crc);

          // same version check
          // there can be case when memory banks are similar but not exactly
          // same meaning some areas were same hence possibly corrupt
          ...
          flush();
          // while (uart_read_bytes(ECHO_UART_PORT_NUM, flush_buf,
          // sizeof(flush_buf),
          //                        10 / portTICK_PERIOD_MS) > 0)
          //   ;
          ...
          if (!wait_for_ack()) {
            ESP_LOGE(TAG, "ee_cmd failed");
            return;
          }

          // Global mass erase: N=0xFFFF, checksum=0x00
          uint8_t global[] = {
              0xFF, 0xFF, 0x00}; // 0xFFFF = Mass erase, 0xFFFE = Bank1 erase
          write_uart(global, 3);
          uint8_t ack;

          // TESTED - Wait for erase complete (can take seconds)
          uint8_t len = uart_read_bytes(ECHO_UART_PORT_NUM, &ack, 1,
          ...
          for (addr_offset = 0; addr_offset < bare_led_bin_len;
            ...
            uart_write_bytes(ECHO_UART_PORT_NUM,
                             (const char *)&bare_led_bin[addr_offset],
                             chunk_len);
          ...
          for (addr_offset = 0; addr_offset < bare_led_bin_len;
            ...
            // const int r = uart_read_bytes(ECHO_UART_PORT_NUM,
            // &current_dump[addr_offset], chunk_len, pdMS_TO_TICKS(1000));
          ...
          if (actual_crc != flash_fw_crc) {
            ...
            /// MASS Extended Erase command - TESTED
            // while (uart_read_bytes(ECHO_UART_PORT_NUM, flush_buf,
            // sizeof(flush_buf), 10 / portTICK_PERIOD_MS) > 0);
            flush();
            ...
            // TESTED - Wait for erase complete (can take seconds)
            len = uart_read_bytes(ECHO_UART_PORT_NUM, &ack, 1,
                                  15000 / portTICK_PERIOD_MS);
            if (len == 1 && ack == 0x79)
              ESP_LOGI(TAG, "Global erase complete!");
            else
            ...
            for (addr_offset = 0; addr_offset < bare_led_bin_len;
                 addr_offset += CHUNK_SIZE)
            // for (addr_offset = 0; addr_offset < bare_led_bin_len; addr_offset
            // += CHUNK_SIZE)
            {
              chunk_len = bare_led_bin_len - addr_offset;
              if (chunk_len > CHUNK_SIZE)
              ...
              uart_write_bytes(ECHO_UART_PORT_NUM, (const char *)&N, 1);
              uart_write_bytes(ECHO_UART_PORT_NUM,
                               (const char *)&full_dump[addr_offset],
                               chunk_len);
              // direct from esp flash
          ...
          uint8_t go_addr[] = {0x08, 0x00, 0x00, 0x00,
                               0x08}; // 0x08000000 + checksum
    ...
    if (event->topic_len == strlen("testtopic/222m") &&
      ...
      cJSON *device = cJSON_GetObjectItem(root, "msg");

      if (cJSON_IsNumber(device)) {
        expected_crc_id = device->valueint;
      }

      ESP_LOGI(TAG, "expected crc %d::%d", expected_crc_id,
               cJSON_IsNumber(device));
      cJSON_Delete(root);
...
static void mqtt_app_start(void) {
  const esp_mqtt_client_config_t mqtt_cfg = {
      .broker = {.address.uri = "mqtt://broker.emqx.io",
                 ...
                 // *)mqtt_eclipseprojects_io_pem_start,
...
void app_main(void) {
  ...
  // Step 2: Flush RX
  // uint8_t flush_buf[64];
  // while (uart_read_bytes(ECHO_UART_PORT_NUM, flush_buf, sizeof(flush_buf), 50
  // / portTICK_PERIOD_MS) > 0)
  //     ;
  /*uint8_t flush_buf[64];
  while (uart_read_bytes(ECHO_UART_PORT_NUM, flush_buf, sizeof(flush_buf), 10 /
  portTICK_PERIOD_MS) > 0) {}*/
  flush();
  ...
  // int len = uart_read_bytes(ECHO_UART_PORT_NUM, &ack, 1, 2000 /
  // portTICK_PERIOD_MS);
  if (len == 1 && ack == 0x79) {
    ...
    // len = uart_read_bytes(ECHO_UART_PORT_NUM, &num_bytes, 1, 2000 /
    // portTICK_PERIOD_MS);
    if (len == 1) {
      ESP_LOGI(TAG, "Bytes to follow: %d", num_bytes);
      uint8_t data[32];
      len = read_uart(data, num_bytes);
      // len = uart_read_bytes(ECHO_UART_PORT_NUM, data, num_bytes, 2000 /
      // portTICK_PERIOD_MS);
      if (len == num_bytes) {
  ...
  int r = read_uart(&full_dump[addr_offset], chunk_len);
  // const int r = uart_read_bytes(ECHO_UART_PORT_NUM, &full_dump[addr_offset],
  // chunk_len, pdMS_TO_TICKS(1000));
  ...
  if (version == -1) {
    ...
    for (addr_offset = 0; addr_offset < bare_led_bin_len;
      ...
      uart_write_bytes(ECHO_UART_PORT_NUM, (const char *)&N, 1);
      uart_write_bytes(ECHO_UART_PORT_NUM,
                       (const char *)&bare_led_bin[addr_offset], chunk_len);
      // direct from esp flash
  ...
  // Global mass erase: N=0xFFFF, checksum=0x00
  uint8_t global[] = {0xFF, 0xFF,
                      0x00}; // 0xFFFF = Mass erase, 0xFFFE = Bank1 erase
  write_uart(global, 3);

  // TESTED - Wait for erase complete (can take seconds)
  len =
      uart_read_bytes(ECHO_UART_PORT_NUM, &ack, 1, 15000 / portTICK_PERIOD_MS);
  if (len == 1 && ack == 0x79)
    ESP_LOGI(TAG, "Global erase complete!");
  else
    ESP_LOGE(TAG, "Erase timeout/NACK: 0x%02X", ack);
  ...
  if (actual_crc != flash_fw_crc) {
    ...
    // CHUNK_SIZE)
    {
      ...
      uart_write_bytes(ECHO_UART_PORT_NUM, (const char *)&N, 1);
      uart_write_bytes(ECHO_UART_PORT_NUM,
                       (const char *)&full_dump[addr_offset], chunk_len);
      // direct from esp flash
